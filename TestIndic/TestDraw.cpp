...
TestDraw::TestDraw(QWidget *parent, Qt::WFlags flags) : AbstractDrawArea(parent, flags),
	LEN(1390), TL(126), scale_k(1)
{
	...
	//this->setFixedSize(area_pic->size());
...
void TestDraw::paintEvent(QPaintEvent * event)
{
	...
	//QPixmap pix(area_pic->width(), area_pic->height());
	//pix = area_pic->copy(0, 0, area_pic->width(), area_pic->height());
	...
	if(inputSignals->contains(INDICATOR_INPUT_SIGNAL))
	{
		...
		if( (abc_len > 0) && (lim >= 0) )
		{
			...
			for(int i = 0; i <= DI; i++)
			{
				...
				float f_ret = (*func_to_visualise)(fst + (abc_len*i/DI), fst, fen) * pow(10., lim/10.);
...
void TestDraw::set_new_mark_h(int new_val)
{
	mark_h = new_val;
	this->repaint();
}
void TestDraw::set_new_m_line(int new_val)
{
	m_line = new_val;
	this->repaint();
}
void TestDraw::set_new_lim(int new_val)
{
	lim = new_val;
	this->repaint();
}