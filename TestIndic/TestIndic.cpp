...
TestIndic::TestIndic(QWidget *parent, Qt::WFlags flags)
  :	AbstractIndicator((AbstractDrawArea *)new TestDraw(parent, flags), parent, flags),
  scale_k(1)
{
...
void TestIndic::initProperties()
{
	set_new_lim(0);
	set_new_m_line(0);
	set_new_mark_h(5);
}
QString TestIndic::getDeviceName()
{
	return QString::fromLocal8Bit("Индикатор КСВН и ослабления Я2Р-67");
}
void TestIndic::paintEvent(QPaintEvent *event)
{
	...
	//QPixmap pix = back->copy(back->rect());
...
void TestIndic::resizeEvent(QResizeEvent *event)
{
...
void TestIndic::set_new_mark_h(int new_val)
{
...
void TestIndic::set_new_m_line(int new_val)
{
...
void TestIndic::set_new_lim(int new_val)
{
...
Q_EXPORT_PLUGIN2(TestIndic, TestIndic);